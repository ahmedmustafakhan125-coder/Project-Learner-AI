...
export function SandboxFrame({
  ...
  /* ---- stable copy of srcdoc so we don't regenerate on every render ---- */
  if (!srcdocRef.current) {
    srcdocRef.current = createSandboxHTML(runtime);
  }
  ...
  /* ---- kill the iframe (used on timeout or unmount) ---- */
  function killFrame() {
    const frame = iframeRef.current;
    if (frame) {
  ...
  useEffect(() => {
    ...
    // handleMessage is stable via useCallback; eslint is happy.
